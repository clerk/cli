...
- Toolchain is Bun (installed under `~/.bun`; on `PATH` via `~/.bashrc`, so use a login shell or call `~/.bun/bin/bun` in non-interactive contexts). The startup update script only refreshes dependencies (`bun install`); everything else is on-demand.
- Standard commands are defined in `package.json` scripts and documented in `CONTRIBUTING.md` — use `bun run lint`, `bun run typecheck`, `bun run test`, and `bun run build`. These need no extra services.
- The product is the `clerk` CLI (no server to run). `bun run dev -- <args>` runs from source, but it executes with `--cwd packages/cli-core`, so cwd-sensitive commands (`doctor`, `init`, `link`, `env`) inspect that package, not your target project. To exercise a cwd-sensitive command against another directory, build once with `bun run build:compile` and run `packages/cli-core/dist/clerk` from inside that directory.
- E2E tests (`bun run test:e2e`) hit the production Clerk API with a dedicated test app: they require the `CLERK_PLATFORM_API_KEY` and `CLERK_CLI_TEST_APP_ID` secrets plus `bunx playwright install chromium`. Without those secrets, stick to the unit suite (`bun run test`). `bun run test:e2e:op` (1Password) is for local human use and will not work here.
