/**
 * User-facing copy for the keyless surface, gathered in one module.
 *
...
export const keylessCopy = {
  ...
  userDashboardNeedsClaim: (keySource: string, userId: string): string =>
    `This directory holds an unclaimed keyless application (secret key from ${keySource}), which has no Dashboard page — a dashboard link needs an application ID, and one is only assigned when the application is claimed.\n` +
