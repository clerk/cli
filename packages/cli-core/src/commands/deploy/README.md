...
| Derive provider domain proxy | `PATCH /v1/platform/applications/{appID}/domain`                            | Sent only when the created domain comes back with `is_provider_domain: true` and no `proxy_url` — `POST .../instances` is the one domain-creating endpoint that does not derive one. Re-sending the current `name` is a server-side no-op for the name but runs the normalization that derives `https://<domain>/__clerk`, validates the domain, and schedules the proxy check. |
...
Which domains may host a production instance is the API's decision, with one
narrow exception.

`POST /v1/platform/applications/{appID}/instances` is the only domain-creating
endpoint in Clerk that does not run `validators.DomainName` — it stores whatever
domain string it is given — and PLAPI exposes no way to delete a production
instance afterwards. A rejected domain would therefore leave the user with an
...
| Update production domain    | `PATCH` | `/v1/platform/applications/{appID}/domain`                               | `updateApplicationDomain`. Used only to make the API derive a provider domain's proxy URL after instance creation. See `commands/deploy/proxy.ts`. |
