...
The active instance is validated against the application's live instance list. Trunk pointers render as `● development (trunk)`, branch pointers as `● feature-auth (branch of development)`. When the pointed-at instance no longer exists (for example, the branch was deleted from another checkout), the line renders as `<label> · instance no longer exists` with a warning that names the fix (`clerk switch`) and notes that `.env.local` still holds the stale keys. When the pointer was set on a different git branch than the current one, a drift warning names both branches and suggests `clerk switch` to re-point.
