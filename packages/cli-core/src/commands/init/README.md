...
Publishable-key discovery is target-aware. The inspector can recognize an inline key passed directly to `Clerk.configure(publishableKey:)`, an enabled Run-scheme environment variable, a target-owned `LocalSecrets.plist`, project `.env` files, and a local Clerk keyless breadcrumb. It distinguishes a key that is available to copy from one that the inspected Swift startup code is known to consume. Output contains only redacted source evidence plus the decoded Frontend API host needed for Associated Domains; it never contains the publishable key itself.
...
16. **Authenticated mode**: pulls development instance API keys via `clerk env pull` for frameworks that consume dotenv files. Native iOS either completes the proven runtime-key handoff or leaves key storage unchanged
