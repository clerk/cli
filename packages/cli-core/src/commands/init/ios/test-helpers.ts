...
function secondTargetObjects(platform: "ios" | "watchos"): string {
  ...
  return `
    ...
    ${IDS.secondDebug} = { isa = XCBuildConfiguration; buildSettings = { GENERATE_INFOPLIST_FILE = YES; ${platformSettings} }; name = Debug; };
    ${IDS.secondRelease} = { isa = XCBuildConfiguration; buildSettings = { GENERATE_INFOPLIST_FILE = YES; ${platformSettings} }; name = Release; };
  `;
}
  ...
  return `// !$*UTF8*$!
{
  ...
  objects = {
    ...
    ${IDS.targetDebug} = { isa = XCBuildConfiguration; ${baseConfigurationReference} buildSettings = { GENERATE_INFOPLIST_FILE = YES; CODE_SIGN_ENTITLEMENTS = MyApp/MyApp.entitlements; ${debugIdentitySettings} ${deploymentTargetSetting} SUPPORTED_PLATFORMS = "${supportedPlatforms}"; }; name = Debug; };
    ${IDS.targetRelease} = { isa = XCBuildConfiguration; ${baseConfigurationReference} buildSettings = { GENERATE_INFOPLIST_FILE = YES; ${releaseEntitlements} ${releaseIdentitySettings} ${deploymentTargetSetting} SUPPORTED_PLATFORMS = "${supportedPlatforms}"; }; name = Release; };
