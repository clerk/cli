...
async function createProcessInfoFixture(
  key: string,
  options: { clerkSDK?: boolean } = {},
): Promise<{ root: string; schemePath: string }> {
  const root = await mkdtemp(join(tmpdir(), "clerk-ios-scheme-runtime-"));
...
`,
  ...
  await Bun.write(schemePath, runSchemeSource(key));
  return { root, schemePath };
}
...
describe("clerk init iOS SDK runtime apply", () => {
  ...
  test("a requested app with a satisfied sink fails closed without its expected key", async () => {
    ...
    await expect(applyIOSPlannedLocalSetup(setup)).rejects.toMatchObject({
      code: ERROR_CODE.IOS_PUBLISHABLE_KEY_UNAVAILABLE,
      message: expect.stringContaining("development publishable key was not available"),
    });
