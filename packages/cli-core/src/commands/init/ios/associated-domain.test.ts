...
describe("iOS Associated Domains setup", () => {
  test("blocks a domain when bounded scheme discovery cannot prove the runtime key", async () => {
    const root = await temporaryRoot();
    await createIOSFixture(root, { includeKey: false, workspace: true });
    await Bun.write(
...
`,
    ...
    await Promise.all(
      Array.from({ length: 99 }, (_, index) =>
        Bun.write(
          join(projectSchemes, `A${String(index).padStart(3, "0")}.xcscheme`),
          "<Scheme />",
        ),
      ),
    );
    await Bun.write(
      join(projectSchemes, "ZRuntime.xcscheme"),
    ...
    const workspaceSchemes = join(root, "MyApp.xcworkspace", "xcshareddata", "xcschemes");
    await mkdir(workspaceSchemes, { recursive: true });
    await Bun.write(
      join(workspaceSchemes, "WorkspaceRuntime.xcscheme"),
