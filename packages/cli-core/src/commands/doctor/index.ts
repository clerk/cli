...
async function runChecks(ctx: DoctorContext, options: DoctorOptions): Promise<CheckResult[]> {
  ...
  const framework = explicitlyRequestsIOS ? { dep: "ios" } : await detectFramework(process.cwd());
  const ios = framework?.dep === "ios";
  const common = await Promise.all(
    getDoctorChecks(ios).map(async (check) => {
      try {
  ...
  try {
    const iosChecks = await runIOSDoctorChecks(ctx, {
      root: process.cwd(),
      ...(options.target ? { target: options.target } : {}),
    });
    const xcodeChecks = await runIOSXcodeVerification(iosChecks.inspection, {
