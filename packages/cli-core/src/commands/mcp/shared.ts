...
// First char alphanumeric/underscore so a name can never parse as a flag.
const ENTRY_NAME_PATTERN = /^[A-Za-z0-9_][A-Za-z0-9_-]{0,63}$/;

/**
 * Resolve the MCP entry name from options, defaulting to
 * {@link DEFAULT_ENTRY_NAME}. Allowlisted rather than escaped: the name is
 * spliced into client CLI argv (on Windows through a `cmd.exe /c` shim where
 * `&`/`|`/`^` are live operators) and used as a config key, and every consumer
 * would need different escaping.
 */
export function resolveName(options: McpOptions): string {
  const name = options.name ?? DEFAULT_ENTRY_NAME;
  if (!ENTRY_NAME_PATTERN.test(name)) {
    throwUsageError(
      `Invalid MCP entry name "${name}". Use letters, digits, dashes, and underscores (must not start with a dash), max 64 characters.`,
    );
  }
  return name;
}
...
export async function targetClients(options: McpOptions, cwd: string): Promise<McpClient[]> {
  ...
  if (detected.length === 0) {
    throw new CliError(
      ...
      { code: ERROR_CODE.MCP_NO_CLIENT_DETECTED, docsUrl: MCP_DOCS_URL },
    );
  }
  return detected;
}
