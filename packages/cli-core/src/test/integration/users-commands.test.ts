...
describe("users commands", () => {
  ...
  test("wizard picker fallback resolves the secret key when no project is linked (regression for 5eed0763)", async () => {
    ...
    // Picker returns app_1; with the forced multi-instance prompt removed
    // (ADR-0011) resolution falls straight to the development root, so no
    // instance picker follows. The wizard then prompts for the optional curated
    // set because MOCK_APP's publishable key does not decode to a valid fapiHost
